...
Orquestra todos os componentes da GUI do gerador de pautas.
"""
...
from gui_widgets import NoteCheckboxPanel, PDFPreviewCanvas, ConfigurationPanel
from pdf_generator import PautaPDFGenerator


class PautaGeneratorGUI(tk.Tk):
    """Classe principal da interface gráfica do gerador de pautas."""
    ...
    def __init__(self):
        ...
        # Caminho para PDF temporário usado no preview
        self.temp_pdf_path = None
        
        # Para debounce do preview automático
        self.preview_update_id = None
        
        # Criar interface
        self._create_widgets()
        
        # Configurar callbacks para preview automático
        self._setup_auto_preview()
        
        # Atualizar preview inicial após um pequeno delay
    ...
    def _setup_auto_preview(self):
        """Configura callbacks para atualização automática do preview."""
        # Callback para mudanças no painel de configurações
        self.config_panel.set_on_change_callback(self._schedule_preview_update)
        
        # Callback para mudanças no painel de notas
        self.note_panel.set_on_change_callback(self._schedule_preview_update)
    
    def _schedule_preview_update(self):
        """
        Agenda uma atualização do preview com debounce.
        Cancela atualizações anteriores pendentes para evitar múltiplas gerações.
        """
        # Cancelar atualização anterior se houver
        if self.preview_update_id:
            self.after_cancel(self.preview_update_id)
        
        # Agendar nova atualização após 500ms de inatividade
        self.preview_update_id = self.after(500, self._update_preview)
    
    def _update_preview(self):
        """Atualiza o preview do PDF (sem mostrar erros para o usuário)."""
        # Validar entradas básicas (sem mostrar mensagens de erro)
        selected_notes = self.note_panel.get_selected_notes()
        if not selected_notes:
            # Limpar preview se não houver notas selecionadas
        ...
        try:
            ...
            # Criar arquivo temporário para preview
            ...
            # Em modo automático, não mostrar erros (pode ser temporário)
            # Apenas limpar preview em caso de erro
            self.preview_canvas.clear()
    
    def _create_right_panel(self, parent):
        """Cria o painel direito com preview."""
        # Título
    ...
    def _on_generate_pdf(self):
        """Handler para gerar o PDF final."""
        # Validar entradas
        ...
        try:
            ...
            # Atualizar preview com o PDF final
    ...
    def __del__(self):
        """Limpa arquivos temporários ao fechar."""
        if self.temp_pdf_path and os.path.exists(self.temp_pdf_path):
            try:
                os.remove(self.temp_pdf_path)
            except:
